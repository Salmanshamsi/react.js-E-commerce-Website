...
const SearchBar = () => {
  ...
  const handleSearch = () => {
    const itemsRef = ref(getDatabase(), 'items');
    const searchQuery = searchText.toLowerCase();

    // Perform the search query
    const query = orderByChild(startAt(endAt(itemsRef, searchQuery + '\uf8ff'), searchQuery), 'name');

    onceValue(query, 'value')
    .then((snapshot) => {
      // ...
    })
    .catch((error) => {
  ...
  return (
    <div>
     ...
     />
        <button type="submit" className="absolute right-0 top-0 mt-5 mr-4" onClick={handleSearch} >
